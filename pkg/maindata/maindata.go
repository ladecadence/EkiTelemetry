...
func (m *Main) Update(t telemetry.Telemetry) {
	...
	m.DataBaro.SetText(t.Baro + " mBar")
	m.DataSats.SetText(fmt.Sprintf("%d", t.Sats))
	m.DataHdg.SetText(t.Hdg + " º")
	m.DataSpd.SetText(t.Spd + " kn")
	m.DataArate.SetText(t.Arate + " m/s")
	m.DataTin.SetText(t.Tin + " ºC")
	m.DataTout.SetText(t.Tout + " ºC")
	m.DataBatt.SetText(t.Vbat + " V")
}
